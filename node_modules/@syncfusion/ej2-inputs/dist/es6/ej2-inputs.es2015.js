...
let NumericTextBox = class NumericTextBox extends Component {
    ...
    formatScientificNotation(value) {
        ...
        const [base, exponentPart] = value.toLowerCase().split('e');
        const exponent = parseInt(exponentPart, 10);
        const match = this.format.match(/n(\d+)/i);
        const requiredDigits = match ? parseInt(match[1], 10) : 10;
        const baseDigits = base.replace('.', '');
        let fullDecimal;
        if (exponent < 0) {
            const zerosCount = Math.abs(Number(exponent)) - 1;
            const limitedZeros = '0'.repeat(Math.min(zerosCount, requiredDigits));
            fullDecimal = '0.' + limitedZeros + baseDigits;
        }
        else {
            const [intPart, decPart = ''] = base.split('.');
            const shift = exponent - decPart.length;
            if (shift >= 0) {
                fullDecimal = intPart + decPart + '0'.repeat(shift);
            }
            else {
                const newDec = decPart.slice(0, exponent) + '.' + decPart.slice(exponent);
                fullDecimal = intPart + newDec;
            }
        }
        const [intPart, decPart = ''] = fullDecimal.split('.');
        if (this.isFocused) {
            const limitedDecimal = decPart.slice(0, requiredDigits);
            return `${intPart}.${decPart}`;
        }
        else {
            const formattedDecimal = decPart.padEnd(requiredDigits, '0').slice(0, requiredDigits);
